...
@if ($addon != null)
    @if($addon->activated)
        @if(in_array(Auth::user()->user_type , ['admin','branch']) || in_array('1001', json_decode(Auth::user()->staff->role->permissions ?? "[]")) || Auth::user()->user_type == 'branch' || Auth::user()->user_type == 'client')
