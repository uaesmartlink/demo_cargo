...
class BusinessSettingsController extends Controller
{
    ...
    public function google_map_update(Request $request)
    {
        ...
        if ($request->has('map')) {
            $google_map = BusinessSetting::where('type', 'google_map')->first();
            $mapbox     = BusinessSetting::where('type', 'mapbox')->first();

            if($request->map == 'googel_map')
            {
                if(isset($request->MAP_KEY))
                {
                    $google_map->key   = $request->MAP_KEY;
                    $google_map->value = 1;
                    $this->overWriteEnvFile('GOOGLE_KEY', $request->MAP_KEY);

                    $mapbox->value = 0;
                }else {
                    flash(translate("Please Enter Google Map Key"))->error();
                    return back();
                }
                
            }elseif($request->map == 'mapbox')
            {
                if(isset($request->mapbox_key))
                {
                    $mapbox->key   = $request->mapbox_key;
                    $mapbox->value = 1;
                    $this->overWriteEnvFile('MAPBOX_KEY', $request->mapbox_key);

                    $google_map->value = 0;

                }else {
                    flash(translate("Please Enter Mapbox Key"))->error();
                    return back();
                }
            }
            ...
            flash(translate("Please Check Google Map Or Mapbox"))->error();
            return back();
        }
    ...
    public function server_key_update(Request $request)
    {
        if (env('DEMO_MODE') == 'On') {
            flash(translate('This action is disabled in demo mode'))->error();
            return back();
        ...
        $business_settings = BusinessSetting::where('type', 'server_key')->first();
        if (empty($request->Server_Key)) {
            flash(translate("Server Key Can Not Be Empty"))->error();
            return back();
        }
        $business_settings->key   = $request->Server_Key;
        $business_settings->save();
        $this->overWriteEnvFile('FCM_SERVER_KEY', $request->Server_Key);

        flash(translate("Server Key updated successfully"))->success();
        return back();
    }
    ...
    public function updateActivationSettings(Request $request)
    {
        ...
        if($business_settings!=null){
            ...
            if ($request->type == 'map_activation' && $request->value == '0') {
                $google_map = BusinessSetting::where('type', 'google_map')->first();
                $mapbox     = BusinessSetting::where('type', 'mapbox')->first();
            ...
            if ($request->value == '0' && ( $request->type == 'nexmo' || $request->type == 'twillo' || $request->type == 'ebernate' || $request->type == 'ssl_wireless' || $request->type == 'fast2sms' || $request->type == 'mimo')) {
                $def_shipment_conf_type = ShipmentSetting::where('key','def_shipment_conf_type')->first();
                $def_shipment_conf_type->value = null;
                $def_shipment_conf_type->save();
            }
