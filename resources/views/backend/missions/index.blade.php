                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right text-danger no-print">
                                   @if(isset($status))
                                    @if($user_type == 'admin' && ($status == \App\Mission::APPROVED_STATUS || $status == \App\Mission::RECIVED_STATUS))
                                    ...
                                    @if($status == \App\Mission::APPROVED_STATUS)
                                        ...
                                        {{-- @endif --}}
                                         @if($user_type == 'captain' || $user_type == 'admin')
                                ...
                                @if(in_array($mission->status_id , [\App\Mission::APPROVED_STATUS,\App\Mission::REQUESTED_STATUS,\App\Mission::RECIVED_STATUS]) && $mission->id != null)
                                    ...
                                    @if($mission->status_id == \App\Mission::RECIVED_STATUS)
                                        @if(($user_type == 'captain' && $mission->getOriginal('type') == \App\Mission::DELIVERY_TYPE) || ($user_type == 'admin' && $mission->getOriginal('type') == \App\Mission::DELIVERY_TYPE))
                                            {{-- @if(Auth::user()->user_type == 'admin' || in_array(1030, json_decode(Auth::user()->staff->role->permissions ?? "[]"))) --}}
                                        ...
                                        @endif
                                        @if($mission->getOriginal('type') == \App\Mission::DELIVERY_TYPE)
                                            <button type="button" class="px-3 mb-1 btn btn-sm btn-primary" data-toggle="modal" data-target="#exampleModalCenter-{{ $mission->id }}" id="modal_open_delete_shipment">
                                                    {{translate('Return')}}
                                                {{-- @else
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.shipments.delete-shipment-from-mission') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="shipment_id" id="delete_shipment_id" value="{{ $mission->shipment_mission[0]->shipment->id }}">
